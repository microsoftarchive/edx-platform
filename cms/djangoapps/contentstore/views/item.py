...
def xblock_handler(request, usage_key_string):
    ...
    if usage_key_string:
        ...
        access_check = has_read_access if request.method == 'GET' else has_write_access
        ...
        if 'duplicate_source_locator' in request.json:
            ...
            if not has_write_access(request.user, parent_usage_key.course_key) or not has_read_access(request.user, duplicate_source_usage_key):
                raise PermissionDenied()
...
def xblock_view_handler(request, usage_key_string, view_name):
    ...
    """
    usage_key = usage_key_with_run(usage_key_string)
    if not has_read_access(request.user, usage_key.course_key):
        raise PermissionDenied()
...
def _create_item(request):
    ...
    if not has_write_access(request.user, usage_key.course_key):
        raise PermissionDenied()
...
def orphan_handler(request, course_key_string):
    ...
    if request.method == 'GET':
        if has_read_access(request.user, course_usage_key):
