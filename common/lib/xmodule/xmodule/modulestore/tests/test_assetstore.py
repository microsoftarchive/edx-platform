...
class TestMongoAssetMetadataStorage(unittest.TestCase):
    ...
    def setup_assets(self, course1_key, course2_key, store=None):
        ...
        asset1_vals = ('pic1.jpg', 'EKMND332DDBK', 'pix/archive', False, 'Author1', datetime.now(pytz.utc), '14', '13')
        asset2_vals = ('shout.ogg', 'KFMDONSKF39K', 'sounds', True, 'Author1', datetime.now(pytz.utc), '1', None)
        asset3_vals = ('code.tgz', 'ZZB2333YBDMW', 'exercises/14', False, 'Author2', datetime.now(pytz.utc), 'AB', 'AA')
        asset4_vals = ('dog.png', 'PUPY4242X', 'pictures/animals', True, 'Author3', datetime.now(pytz.utc), '5', '4')
        asset5_vals = ('not_here.txt', 'JJJCCC747', '/dev/null', False, 'Author4', datetime.now(pytz.utc), '50', '49')
        ...
        asset6_vals = ('asset.txt', 'JJJCCC747858', '/dev/null', False, 'Author4', datetime.now(pytz.utc), '50', '49')
        ...
        if store is not None:
            self.assertTrue(store.save_asset_metadata(course1_key, asset1_md))
            self.assertTrue(store.save_asset_metadata(course1_key, asset2_md))
            self.assertTrue(store.save_asset_metadata(course2_key, asset3_md))
            self.assertTrue(store.save_asset_metadata(course2_key, asset4_md))
    ...
    def setup_thumbnails(self, course1_key, course2_key, store=None):
        ...
        if store is not None:
            self.assertTrue(store.save_asset_thumbnail_metadata(course1_key, thumb1_md))
            self.assertTrue(store.save_asset_thumbnail_metadata(course1_key, thumb2_md))
            self.assertTrue(store.save_asset_thumbnail_metadata(course2_key, thumb3_md))
            self.assertTrue(store.save_asset_thumbnail_metadata(course2_key, thumb4_md))
    ...
    def test_save_one_and_confirm(self, storebuilder):
        ...
        with MongoContentstoreBuilder().build() as contentstore:
            with storebuilder.build(contentstore) as store:
                ...
                self.assertTrue(store.save_asset_metadata(course.id, new_asset_md))
                ...
                store.delete_all_asset_metadata(course.id)
    ...
    def test_lock_unlock_assets(self, storebuilder):
        ...
        with MongoContentstoreBuilder().build() as contentstore:
            with storebuilder.build(contentstore) as store:
                ...
                store.set_asset_metadata_attr(new_asset_loc, ModuleStoreEnum.UserID.test, "locked", not locked_state)
                ...
                # Now flip it back.
                store.set_asset_metadata_attr(new_asset_loc, ModuleStoreEnum.UserID.test, "locked", locked_state)
    ...
    def test_set_all_attrs(self, storebuilder):
        ...
        with MongoContentstoreBuilder().build() as contentstore:
            with storebuilder.build(contentstore) as store:
                ...
                new_asset_md = self._make_asset_metadata(new_asset_loc)
                self.assertTrue(store.save_asset_metadata(course.id, new_asset_md))
                for attr_pair in self.ALLOWED_ATTRS:
                    # Set the course asset's attr.
                    store.set_asset_metadata_attr(new_asset_loc, ModuleStoreEnum.UserID.test, *attr_pair)
    ...
    def test_set_disallowed_attrs(self, storebuilder):
        ...
        with MongoContentstoreBuilder().build() as contentstore:
            with storebuilder.build(contentstore) as store:
                ...
                new_asset_md = self._make_asset_metadata(new_asset_loc)
                self.assertTrue(store.save_asset_metadata(course.id, new_asset_md))
                for attr_pair in self.DISALLOWED_ATTRS:
                    original_attr_val = getattr(new_asset_md, attr_pair[0])
                    # Set the course asset's attr.
                    store.set_asset_metadata_attr(new_asset_loc, ModuleStoreEnum.UserID.test, *attr_pair)
    ...
    def test_set_unknown_attrs(self, storebuilder):
        ...
        with MongoContentstoreBuilder().build() as contentstore:
            with storebuilder.build(contentstore) as store:
                ...
                new_asset_md = self._make_asset_metadata(new_asset_loc)
                self.assertTrue(store.save_asset_metadata(course.id, new_asset_md))
                for attr_pair in self.UNKNOWN_ATTRS:
                    # Set the course asset's attr.
                    store.set_asset_metadata_attr(new_asset_loc, ModuleStoreEnum.UserID.test, *attr_pair)
                    # Find the same course and check its changed attr.
                    updated_asset_md = store.find_asset_metadata(new_asset_loc)
    ...
    def test_save_one_thumbnail_and_delete_one_thumbnail(self, storebuilder):
        ...
        with MongoContentstoreBuilder().build() as contentstore:
            with storebuilder.build(contentstore) as store:
                ...
                self.assertTrue(store.save_asset_thumbnail_metadata(course.id, new_asset_thumbnail))
    ...
    def test_find_thumbnail(self, storebuilder):
        ...
        with MongoContentstoreBuilder().build() as contentstore:
            with storebuilder.build(contentstore) as store:
                ...
                self.assertIsNotNone(store.find_asset_thumbnail_metadata(new_asset_thumbnail))
    ...
    def test_asset_object_equivalence(self):
        ...
        """
        course1_key = CourseLocator("org", "course", "run1")
        course2_key = CourseLocator("org", "course", "run2")
        self.setup_assets(course1_key, course2_key, None)
        self.setup_thumbnails(course1_key, course2_key, None)
