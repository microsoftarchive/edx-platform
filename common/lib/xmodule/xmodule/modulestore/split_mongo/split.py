...
class SplitBulkWriteMixin(BulkOperationsMixin):
    ...
    def get_cached_block(self, course_key, block_id):
        """
        ...
        bulk_write_record = self._get_bulk_ops_record(course_key)
        if bulk_write_record.active:
            return bulk_write_record.modules[course_key.version_guid].get(block_id, None)

    def cache_block(self, course_key, block_key, block):
        """
        ...
        bulk_write_record = self._get_bulk_ops_record(course_key)
        if bulk_write_record.active:
            bulk_write_record.modules[course_key.version_guid][block_key] = block

    def decache_block(self, course_key, block_key):
        """
        ...
        if bulk_write_record.active:
            try:
                del bulk_write_record.modules[course_key.version_guid][block_key]
            except KeyError:
                pass
...
class SplitMongoModuleStore(SplitBulkWriteMixin, ModuleStoreWriteBase):
    ...
    def create_child(self, user_id, parent_usage_key, block_type, block_id=None, fields=None, **kwargs):
        ...
        with self.bulk_operations(parent_usage_key.course_key):
            ...
            self.decache_block(parent_usage_key.course_key, block_id)
    ...
    def delete_item(self, usage_locator, user_id, force=False):
        ...
        with self.bulk_operations(usage_locator.course_key):
            ...
            if parent_block_key:
                ...
                self.decache_block(usage_locator.course_key, parent_block_key)
