...
from src.xblock.xblock.core import XBlock
...
class SplitMongoKVS(InheritanceKeyValueStore):
    ...
    def __init__(self, definition, initial_values, parent, aside_fields, field_decorator=None):
        """
        ...
        self.parent = parent
        self.aside_fields = aside_fields
    ...
    def delete(self, key):
        ...
        if key.block_family == XBlockAside.entry_point:
            ...
            if key.block_scope_id.block_type in self.aside_fields and key.field_name in self.aside_fields[key.block_scope_id.block_type]:
    ...
    def has(self, key):
        ...
        if key.scope == Scope.content:
            self._load_definition()
        elif key.scope == Scope.parent:  # Is this true for detached categories? (about, course-info, tabs)
            return True
        ...
        if key.block_family == XBlockAside.entry_point:
            ...
            return key.block_scope_id.block_type in self.aside_fields and key.field_name in self.aside_fields[key.block_scope_id.block_type]
        else:
    ...
    def _load_definition(self):
        ...
        if isinstance(self._definition, DefinitionLazyLoader):
            ...
            if persisted_definition is not None:
                ...
                aside_fields = self._definition.field_converter(persisted_definition.get('aside_fields'))
