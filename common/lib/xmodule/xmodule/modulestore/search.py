...
def navigation_index(position):
    ...
    Argument:
    position - result of position returned from call to path_to_location
    ...
    try:
        # position - is an underscore (_) separated string of vertical 1-indexed positions. If the course is built in
        # Studio then you'll never see verticals as children of verticals, and so extremely often one will only see the
        # first vertical as an integer position. This specific action is to allow navigation / breadcrumbs to locate
        # the topmost item because this is the location actually required by the LMS code
